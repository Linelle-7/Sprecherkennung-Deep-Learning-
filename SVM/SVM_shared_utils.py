...
import numpy as np, pandas as pd, os, sounddevice as sd, matplotlib.pyplot as plt, seaborn as sns, warnings
...
def objective(trial,X_train, y_train):
    ...
    with parallel_backend("threading"):
        C = trial.suggest_float("C", 1e-2, 100, log=True)  # Logarithmischer Bereich
        kernel = trial.suggest_categorical("kernel", ["linear", "rbf", "poly"])
        gamma = trial.suggest_categorical('gamma', [0.1, 0.01, 0.001, 'scale','auto'])
...
def live_audio_analysis_svm2(model, scaler, label_map, segment_length=0.1, sr=16000, window_size=3):
    """
    Führt eine Live-Sprechererkennung mit einem SVM-Modell durch und glättet die Ergebnisse.
    ...
    segment_samples = int(segment_length * sr)
    current_audio = np.zeros(0, dtype=np.float32)
    history = deque(maxlen=window_size)  # File glissante pour le lissage

    def callback(indata, frames, time, status):
        if status:
            print(status)
        buffer.put(indata[:, 0])  # Stocke les données audio

    def format_time(seconds):
        """Format seconds into mm:ss:msms."""
        ...
        try:
            ...
            while True:
                ...
                while len(current_audio) >= segment_samples:
                    ...
                    # Feature extraction et mise à l'échelle
                    mfccs = extract_features(segment, sr)  # Fonction existante
                    features_scaled = scaler.transform([mfccs])

                    # Prédiction avec SVM
                    predicted_label = model.predict(features_scaled)[0]
                    history.append(predicted_label)  # Ajout à la file pour le lissage

                    # Affichage de la prédiction brute
                    speaker_name = label_to_name.get(predicted_label, "Unbekannt")
                    print(f"[{format_time(start_time)} - {format_time(start_time + segment_length)}] {speaker_name}")
                    start_time += segment_length

                    # Lissage des résultats si la fenêtre est remplie
                    if len(history) == window_size:
                        counter = Counter(history)
                        most_common_label, count = counter.most_common(1)[0]

                        # Seuil de confiance à 60%
                        if count / window_size >= 0.6:
                            smoothed_label = most_common_label
                        else:
                            smoothed_label = "Inconnu"

                        smoothed_speaker_name = label_to_name.get(smoothed_label, "Unbekannt")
                        print(f"Glättung: {smoothed_speaker_name}")
